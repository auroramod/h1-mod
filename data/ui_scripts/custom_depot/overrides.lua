...
Scoreboard = function(unk1, unk2)
    ...
    local added_currency = false
    local ui_round_end_orig = scoreboard.m_eventHandlers["ui_round_end"]

    scoreboard:registerOmnvarHandler("ui_round_end", function(unk3, unk4)
        if not added_currency then
            local player_stats = Game.GetPlayerScoreInfoAtRank(Game.GetPlayerTeam(), Game.GetPlayerScoreRanking())

            custom_depot.get_function("add_currency")(InventoryCurrencyType.Parts,
                math.floor(player_stats.extrascore0 * 10 / 100))

            custom_depot.get_function("save_depot_data")()
            added_currency = true
        end

        ui_round_end_orig(unk3, unk4)
    end)
