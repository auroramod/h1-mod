...
namespace dvars
{
	...
	{
		{"accessToSubscriberContent", "Whether to display the subscriber maps."},
		{"aci", ""},
		{"actionSlotsHide", "Hide the actionslots."},
		{"ai_grenadeReturn_approachMinDot", "Minimal dot product between the approach and throw vectors to perform a grenade return"},
		{"ai_grenadeReturn_debug", "Turns on debug info for AI grenade returns"},
		{"ai_grenadeReturn_extraFuseTime", "The amount of time (in ms) to add to a grenade fuse when trying to return grenade that's below minFuseTime"},
		{"ai_grenadeReturn_minDistSqr", "Minimal distance to a grenade to consider it for a return so that transition anims will play"},
		{"ai_grenadeReturn_minFuseTime", "If the fuse time drops below this value when an ally is attempting to return a grenade, add extra fuse time"},
		{"ai_grenadeReturn_stationary", "If set, AI will attempt to return grenades that they are within pickup distance - regardless of min dist"},
		{"ai_grenadeReturn_traceToGrenade", "If set, AI will only attempt to return grenades when they have a clear sight trace to the grenade"},
		...
		{"bg_allowScuffFootsteps", "If true, scuff sounds will be played when the player rotates in place."},
		...
		{"bg_compassShowEnemies", "Whether enemies are visible on the compass at all times"},
		{"bg_idleSwingSpeed", "The rate at which the player's legs swing around when idle (multi-player only)"},
		...
		{"bg_shock_fadeOverride", "Override the time for the shellshock kick effect to fade in MP"},
		...
		{"bg_shock_screenBlurBlendTime", "The amount of time in seconds for the shellshock effect to fade"},
		{"bg_shock_screenFlashShotFadeTime", "In seconds, how soon from the end of the effect to start blending out the whiteout layer."},
		...
		{"bg_shock_soundRoomType", "Shell shock sound reverb room type"},
		{"bg_shock_soundSubmix", "Shell shock submix to apply"},
		...
		{"bg_swingSpeed", "The rate at which the player's legs swing around when idle (multi-player only)"},
		{"bg_torsoSwingSpeed", "The rate at which the player's torso swings around when strafing (multi-player only)"},
		...
		{"bot_DifficultyDefault", "default difficulty level of bots"},
		{"ca_auto_signin", "CoD Anywhere start sign-in task automatically on startup or first party sign-in"},
		{"ca_do_mlc", "CoD Anywhere Do Multi Login check"},
		{"ca_intra_only", "CoD Anywhere Intra Network Only"},
		{"ca_require_signin", "CoD Anywhere require sign in to enter MP"},
		{"ca_show_signup_request", "CoD Anywhere should you show new users a popup requesting they create a CoD Account?"},
		...
		{"cg_airstrikeCamFstop", "Airstrike kill camera aperture. Lower f-stop yields a shallower depth of field. Typical values range from 1 to 22"},
		...
		{"cg_blood", "Show Blood"},
		{"cg_bloodThickColor", "Color of the blood overlay's thick blood splatter"},
		{"cg_bloodThinColor", "Color of the blood overlay's thin blood splatter"},
		...
		{"cg_ColorBlind_EnemyTeam", "Enemy team color for color blind people"},
		{"cg_ColorBlind_MyParty", "Player party color for color blind people"},
		{"cg_ColorBlind_MyTeam", "Player team color for color blind people"},
		...
		{"cg_drawBuildName", "Draw build name"},
		...
		{"cg_drawDoubleTapDetonateHint", "Draw a 'double tap to detonate grenade' hint"},
		{"cg_drawEffectNum", "Draw counts of effects and elements"},
		{"cg_drawFPS", "Draw frames per second"},
		{"cg_drawFPSLabels", "Draw FPS Info Labels"},
		{"cg_drawFriendlyHUDGrenades", "Draw grenade warning indicators for friendly grenades (should be true if friendly-fire is enabled)"},
		{"cg_drawFriendlyNames", "Whether to show friendly names in game"},
		{"cg_drawFriendlyNamesAlways", "Whether to always show friendly names in game (for certain gametypes)"},
		...
		{"cg_e3TrailerHacks", "Tweaks for trailer recording"},
		{"cg_equipmentSounds", "Play equipment sounds"},
		...
		{"cg_explosiveKillCamGroundUpDist", "Explosive kill camera when stuck to ground: distance of camera backwards from explosive."},
		{"cg_explosiveKillCamStopDecelDist", "Rocket and Grenade Launcher kill camera: distance from player to begin coming to rest"},
		{"cg_explosiveKillCamStopDist", "Rocket and Grenade Launcher kill camera: distance from player to begin coming to rest"},
		{"cg_explosiveKillCamUpDist", "Explosive kill camera: distance of camera backwards from explosive."},
		{"cg_explosiveKillCamWallOutDist", "Explosive kill camera when stuck to wall: distance of camera out from wall."},
		{"cg_explosiveKillCamWallSideDist", "Explosive kill camera when stuck to wall: distance of camera out from wall."},
		{"cg_flashbangNameFadeIn", "Time in milliseconds to fade in friendly names"},
		{"cg_flashbangNameFadeOut", "Time in milliseconds to fade out friendly names"},
		{"cg_foliagesnd_alias", "The sound that plays when an actor or player enters a foliage clip brush."},
		{"cg_footsteps", "Play footstep sounds that are NOT sprint"},
		{"cg_footstepsSprint", "Play sprint footstep sounds"},
		...
		{"cl_demo_uploadfb", "Should we upload to FB"},
		{"cl_dirSelConvergenceTime", "Time to converge to the new direction when selecting a direction on the map."},
		{"cl_force_paused", "Force the client to be paused. Can't be overridden by LUA scripts, the start button, etc."},
		...
		{"cl_inhibit_stats_upload", "Inhibit upload of stats during demo playback"},
		{"cl_lessprint", "Print less to the console by filtering out certain spammy channels"},
		{"cl_maxpackets", "Maximum number of packets sent per frame"},
		{"cl_maxPing", "Maximum ping for the client"},
		{"cl_migrationTimeout", "Seconds to wait to hear from new host during host migration before timeout occurs"},
		{"cl_modifiedDebugPlacement", "Modify the location of debug output (outside of safe area)"},
		{"cl_motdString", ""},
		{"cl_mouseAccel", "Mouse acceleration"},
		{"cl_noprint", "Print nothing to the console"},
		{"cl_packetdup", "Enable packet duplication"},
		{"cl_pauseAudioZoneEnabled", "Enable the paused audio zone when the menus are up"},
		{"cl_paused", "Pause the game"},
		{"cl_pitchspeed", "Max pitch speed in degrees for game pad"},
		{"cl_pranks", "pranks"},
		{"cl_pushToTalk", "Do we have to press a button to talk"},
		...
		{"cl_textChatEnabled", "Do we want to use text chat"},
		...
		{"cod_anywhere_errorMessage", "CoD Anywhere error message"},
		{"cod_anywhere_showPopup", "Temp Development: Should we show the CoD Anywhere popup"},
		{"cod_anywhere_single_task_popup_text", "CoD Anywhere success message"},
		{"com_animCheck", "Check anim tree"},
		{"com_cinematicEndInWhite", "Set by script. True if cinematic ends with a white screen."},
		{"com_completionResolveCommand", "Command to run when the message box successfully closes"},
		...
		{"com_playerProfile", "Set to the name of the profile"},
		{"com_recommendedSet", ""},
		{"commerce_dl_retry_step", "Step in m/s for the commerce download retry"},
		{"commerce_manifest_file_max_retry_time", "Max time that the commerce manifest can retry"},
		{"commerce_manifest_file_retry_step", "Step in m/s for the commerce manifest retry"},
		{"commerce_max_dl_retry_time", "Max time that the commerce download can retry"},
		{"commerce_max_retry_time", "Max time that the commerce upload can retry"},
		{"commerce_retry_step", "Step in m/s for the commerce upload retry"},
		...
		{"compassObjectiveNearbyDist", "When an objective is closer than this distance (in meters), the icon will not be drawn on the tickertape."},
		...
		{"compassObjectiveWraparoundTime", "How long it takes for the objective to wrap around the compass from one edge to the other"},
		{"compassPlayerHeight", ""},
		{"compassPlayerWidth", ""},
		...
		{"comscore_active", "Are we allowed to enable ComScore tracking or not"},
		...
		{"counterDownloadInterval", "Number of minutes before all the global counters are uploaded"},
		{"counterUploadInterval", "Number of minutes before all the global counters are uploaded"},
		{"cpu_speed_12players", "12 player sys_configureGHz req'd"},
		{"cpu_speed_18players", "18 player sys_configureGHz req'd"},
		{"cpu_speed_8players", "8 player sys_configureGHz req'd"},
		{"cSplineDebugRender", "Debug Render the csplines."},
		{"cSplineDebugRenderCorridor", "Debug Render the cspline corridor."},
		{"cSplineDebugRenderData", "Debug Render the cspline data."},
		{"cSplineDebugRenderSplineId", "Select a cspline - 0 for all."},
		{"dailychallenge_killswitch", "daily challenge killswitch - int with bits used to flag individual daily challenges as enabled"},
		{"dailychallenge_killswitch2", "daily challenge killswitch2 - int with bits used to flag 2nd set of individual daily challenges as enabled"},
		{"dailychallenge_period", "daily challenge period - utc value for a day"},
		{"data_validation_allow_drop", ""},
		{"dc_lobbymerge", "Allows lobby merging across data centres"},
		{"dcacheSimulateNoHDD", "When turned on, simulate no HDD for caching."},
		{"dcacheThrottleEnabled", "Enable or disable dcache upload throttling."},
		{"dcacheThrottleKBytesPerSec", "Dcache upload throttle limit in K Bytes per second."},
		{"dedicated_dhclient", "True if we're a client playing on a DH server"},
		{"demonwareConsideredConnectedTime", "Number of milliseconds after being disconnected from demonware before considering shutting down."},
		{"developer", "Enable development options"},
		{"didyouknow", ""},
		{"discard_playerstats_on_suspend", "Forces stats discard on suspend"},
		{"drawEntityCount", "Enable entity count drawing"},
		{"drawEntityCountPos", "Where to draw the entity count graph"},
		{"drawEntityCountSize", "undefined"},
		...
		{"ds_dcid", "optional datacenter id - from playlist"},
		{"ds_dcid_override", "force datacenter id"},
		{"ds_info", "ds info string"},
		{"ds_info_enable", "Enable ds info string"},
		{"ds_introRequestTimeout", "ds intro request timeout (ms)"},
		{"ds_keepaliveInterval", "ds keepalive interval (ms)"},
		{"ds_keepaliveTimeout", "ds keepalive timeout (ms)"},
		{"ds_pingclient_max_reping_distance", "don't re-ping a datacenter if it's further away than this (miles)"},
		{"ds_pingclient_max_repings", "max # of times to re-ping a datacenter"},
		{"ds_pingclient_maxpings", "max pings to send per datacenter"},
		{"ds_pingclient_maxpings_per_tick", "max new pings each tick"},
		{"ds_pingclient_min_reping_delay", "min msec delay between re-pings"},
		{"ds_pingclient_min_reping_latency", "don't re-ping a datacenter if latency is less than this"},
		{"ds_pingclient_minpings", "min responses required per datacenter"},
		{"ds_pingclient_odsf", "does dsping set odsf flag"},
		{"dsping_dc_0", ""},
		{"dsping_dc_1", ""},
		{"dsping_dc_10", ""},
		{"dsping_dc_11", ""},
		...
		{"dsping_dc_8", ""},
		{"dsping_dc_9", ""},
		{"dvl", "Enables the data validation system. Only available in non-retail builds."},
		{"dw_addrHandleTimeout", "Delay before destroying an addrHandle after the connection is lost\n"},
		{"dw_leaderboard_write_active", "Are leaderboard writes enabled"},
		{"dw_presence_active", "Is the demonware presence system enabled"},
		{"dw_presence_coop_join_active", "Do we allow players to join on presence for private coop matches (post session to demonware"},
		{"dw_presence_get_delay", "Number of milliseconds to wait after booting the game to fetch demonware presence"},
		{"dw_presence_get_rate", "Number of milliseconds to wait between sending presence state to demonware"},
		{"dw_presence_put_delay", "Number of milliseconds to wait in a presence state before sending to demonware"},
		{"dw_presence_put_rate", "Number of milliseconds to wait between sending presence state to demonware"},
		{"dw_region_lookup_timeout", "Timeout (in MS) after which we will accept not having found a region code and use the default"},
		{"dw_shared_presence_active", "Is the demonware shared presence system enabled"},
		{"dw_shared_presence_get_delay", "Number of milliseconds to wait after booting the game to fetch demonware presence"},
		{"dw_shared_presence_get_rate", "Number of milliseconds to wait between sending presence state to demonware"},
		{"dw_shared_presence_put_delay", "Number of milliseconds to wait in a shared presence state before sending to demonware"},
		{"dw_shared_presence_put_rate", "Number of milliseconds to wait between sending presence state to demonware"},
		{"dwBandwidthTestTaskTimeout", "default timeout for the bandwidth test task (in ms). 0 means no timeout"},
		...
		{"facebook_password_asterisk", "Facebook Password (Asterisk Version)"},
		{"facebook_popup_text", "Facebook Popup Text"},
		{"facebook_retry_step", "Step in m/s for the Facebook authentication retry"},
		{"facebook_upload_photo_active", "Are we allowed to Upload Photos to  Facebook or not"},
		{"facebook_upload_video_active", "Are we allowed to Upload Videos to Facebook or not"},
		...
		{"friendsCacheSteamFriends", "Use cache of steam friends before querying steam api"},
		{"friendsMaxSteamLookupsPerFrame", "Number of steam friends to query steam status per frame when doing a refresh.\n"},
		...
		{"fs_basepath_output", "Base game path"},
		{"fs_cdpath", "CD path"},
		...
		{"fx_alphaThreshold", "Don't draw billboard sprites, oriented sprites or tails with alpha below this threshold (0-256)."},
		{"fx_cast_shadow", "Enable transparency shadow mapping from script"},
		{"fx_count", "Debug effects count"},
		{"fx_cull_elem_draw", "Culls effect elems for drawing"},
		{"fx_cull_elem_draw_flicker", "Flicker DPVS culled effect elems"},
		{"fx_cull_elem_spawn", "Culls effect elems for spawning"},
		{"fx_debugBolt", "Debug effects bolt"},
		{"fx_deferelem", "Toggles deferred processing of elements instead of effects"},
		{"fx_dpvs_cull_elem_draw", "Culls effect elems for drawing using DPVS(2: ignore per-effect portal culling flag)"},
		{"fx_draw", ""},
		{"fx_draw_omniLight", ""},
		{"fx_draw_simd", "Draw effects using SIMD / Vector code."},
		{"fx_draw_spotLight", ""},
		{"fx_drawClouds", "Toggles the drawing of particle clouds"},
		{"fx_enable", "Toggles all effects processing"},
		{"fx_flare", "Toggles fx flare"},
		{"fx_freeze", "Freeze effects"},
		{"fx_killEffectOnRewind", "Causes effects that have been marked for a soft kill (fade out) to be killed immediately on a rewind."},
		{"fx_lightGridSampleOffset", "the length of effect sample's offset along X Axis"},
		...
		{"fx_marks_ents", "Toggles whether bullet hits leave marks"},
		{"fx_marks_nearlimit", "Sets limit of number of decals that can exist at the same location (0 for unlimited)"},
		{"fx_marks_smodels", "Toggles whether bullet hits leave marks"},
		{"fx_physicsImpactVelocityThreshold", "Set the min normal velocity threshold in order for model physics fx to generate child impact effects."},
		...
		{"fx_profileFilterElemCountZero", "Do not include FX that have a zero element count"},
		...
		{"fx_showLightGridSampleOffset", "show light grid sample offset in CreateFX mode"},
		{"fx_visMinTraceDist", "Minimum visibility trace size"},
		{"g_allowVote", "Enable voting on this server"},
		{"g_atmosFogDistanceScaleReadOnly", "scale applied to scene distance used for atmospheric fog calculation"},
		{"g_atmosFogEnabledReadOnly", "use atmospheric fog"},
		...
		{"laserLightRadius", "undefined"},
		{"laserRadius", "undefined"},
		...
		{"lb_throttle_time", "Lobby throttling amount"},
		{"lb_times_in_window", "Lobby throttling window amount"},
		{"lb_window", "Lobby throttling window"},
		{"live_qosec_firstupdatems", "MS to wait before deciding to early out qos"},
		{"live_qosec_lastupdatems", "MS since last update required to early out qos"},
		{"live_qosec_minpercent", "Minimum percentage of probe results required before early outing qos"},
		{"live_qosec_minprobes", "Minimum probe results required before early outing qos"},
		{"liveanticheatunknowndvar", "Live Anti Cheat Unknown Dvar"},
		{"livestreaming_active", "Are we allowed to enable LiveStreaming or not"},
		{"loading_sre_fatal", "Loading errors prevent level from loading."},
		...
		{"lowAmmoWarningNoReloadColor1", "Like lowAmmoWarningColor1, but when no ammo."},
		{"lowAmmoWarningNoReloadColor2", "lowAmmoWarningColor2, but when no ammo."},
		...
		{"lsp_enumertion_max_retry_time", "Max time that the LSP enumeration can retry"},
		{"lsp_enumertion_retry_step", "Step in m/s for the LSP enumeration retry"},
		{"lui_demoMode", "Check if the game is in demo mode."},
		{"lui_FFotDSupportEnabled", "Enables lui to update itself via the ffotd"},
		{"lui_hud_motion_angle_ease_speed", "Hud motion ease percentage of degrees per second"},
		{"lui_hud_motion_bob_scale", "Hud motion bob scale"},
		{"lui_hud_motion_enabled", "Enable hud motion"},
		{"lui_hud_motion_perspective", "value for hud motion perspective transform in pixels"},
		{"lui_hud_motion_rotation_max", "Hud motion rotation max"},
		{"lui_hud_motion_rotation_scale", "Hud motion rotation scale"},
		{"lui_hud_motion_trans_ease_speed", "Hud motion ease percentage of pixels per second"},
		{"lui_hud_motion_translation_max", "Hud motion translation max"},
		{"lui_hud_motion_translation_scale", "Hud motion translation scale"},
		{"lui_loot_duplicateredemption", "Whether a user can redeem duplicate loot items in the Armory"},
		{"LUI_MemErrorsFatal", "Out of memory errors cause drops when true, reinits the UI system if false"},
		{"lui_menuFlowEnabled", "Enables LUI menu flow"},
		{"lui_mlg_rules_unlocked", "Whether MLG rules are unlocked"},
		{"lui_priv_lobby_team", "Team selected in private match lobby"},
		{"lui_splitscreensignin_menu", "Enables the LUI splitscreensignin menu"},
		{"lui_splitscreenupscaling", "Force splitscreen upscaling off/on (-1 off, 1 on) -- requires map change"},
		{"lui_systemlink_menu", "Enables the LUI systemlink menu"},
		{"lui_waitingforgavelmessagesconfirmed", ""},
		{"lui_waitingfornetworktype", "value is LuiWaitingForNetworkType enum"},
		{"lui_waitingforonlinedatafetch_controller", "the controller index that is fetching the online stats data"},
		{"LUI_WorkerCmdGC", "Dev-only flag to enable/disable LUI workerCmd GC thread"},
		{"lui_xboxlive_menu", "Enables the LUI xboxlive menu"},
		...
		{"m_yaw", "Default yaw"},
		{"manifestfs", "Use a manifest file to read segmented fastfiles"},
		{"mapname", "The current map name"},
		{"mapPackMPGroupFourFlags", "Map pack flags that comprise MP ala carte map pack 1"},
		{"mapPackMPGroupFreeFlags", "Map pack flags that comprise the free MP ala carte map pack"},
		{"mapPackMPGroupOneFlags", "Map pack flags that comprise MP ala carte map pack 1"},
		{"mapPackMPGroupThreeFlags", "Map pack flags that comprise MP ala carte map pack 1"},
		{"mapPackMPGroupTwoFlags", "Map pack flags that comprise MP ala carte map pack 1"},
		{"marketing_active", "Are we allowed to enable Marketing Comms or not"},
		{"marketing_refresh_time", "time in seconds to wait before refreshing marketing messages from demonware"},
		{"matchdata_active", "Are match data uploads enabled"},
		{"matchdata_maxcompressionbuffer", "Max SP match data compression buffer to use (in bytes)"},
		{"matchmaking_debug", "Enable matchmaking debugging information"},
		{"max_ping_threshold_good", "max ping value to be considered as good"},
		{"max_ping_threshold_medium", "max ping value to be considered as medium"},
		{"max_xp_per_match", ""},
		{"maxPrestigeOverride", "Overrides the maximum prestige level, disabled if 0."},
		{"maxVoicePacketsPerSec", ""},
		{"maxVoicePacketsPerSecForServer", ""},
		{"mdsd", "enable match data stat delta logging?"},
		...
		{"party_alternateMapVoteStatus", "Alternate map vote progress"},
		...
		{"party_mapvote_entrya_mapname", "Primary map vote entry name"},
		{"party_mapvote_entryb_mapname", "Alternate map vote entry name"},
		...
		{"party_nextMapVoteStatus", "Next map vote progress"},
		{"party_partyPlayerCount", "Number of players currently in the party/lobby"},
		{"party_partyPlayerCountNum", "Number of players currently in the party/lobby"},
		{"party_playersCoop", "True if the current playlist places all players on the allies team"},
		{"party_playervisible", "Whether selected player in party is showing true info or not.  Only valid when used with party feeders."},
		{"party_randomMapVoteStatus", "Random map vote progress"},
		...
		{"perk_quickDrawSpeedScaleSniper", "Scales the 'Hip to ADS' transition speed."},
		{"perk_scavengerMode", ""},
		...
		{"physVeh_minContactImpulse", "The minimum impulse needed to register a contact notification"},
		...
		{"playercard_cache_download_max_retry_time", "Max time that the player cache download can retry"},
		{"playercard_cache_download_retry_step", "Step in m/s for the player cache download retry"},
		{"playercard_cache_upload_max_retry_time", "Max time that the player cache upload can retry"},
		{"playercard_cache_upload_retry_step", "Step in m/s for the player cache upload retry"},
		{"playercard_cache_validity_life", "The life of a cached gamercard (it can be re-downloaded after this)"},
		{"playerPositionRecordSampleTime", "How often to sample player positions and save them into match data."},
		{"playlist", "The playlist number"},
		{"playlistAggrFilename", "Aggregated playlist filename"},
		...
		{"r_dof_physical_bokehPreset", "Changes dof sampling quality"},
		...
		{"r_dof_physical_hipEnable", "Enable hyperfocal mode"},
		...
		{"r_dof_physical_minFocusDistance", "Minimum focus distance (inches)"},
		...
		{"r_hemiAoBlurTolerance", "Hemi SSAO Blur Tolerance (log10)"},
		...
		{"r_hemiAoEnable", "Enable Hemi SSAO"},
		{"r_hemiAoHierarchyDepth", "Hemi SSAO recursion depth (filter width)"},
		...
		{"r_hemiAoQualityLevel", "Hemi SSAO quality setting"},
		...
		{"r_hemiAoUpsampleTolerance", "Hemi SSAO Upsample Tolerance (log10)"},
		{"r_heroLighting", "Enable hero-only lighting"},
		{"r_highLodDist", "Distance for high level of detail"},
		{"r_hudFx", "Draw HUD Effects"},
		{"r_hudOutlineEnable", "Enables wireframe outlines to be drawn around DObjs (as a post process)."},
		{"r_hudOutlinePostMode", "hud outline apply mode"},
		...
		{"r_nvidiaGPU", "Enable NV API."},
		{"r_offchipTessellationAllowed", "Enable off-chip tessellation support."},
		{"r_offchipTessellationTfThreshold", "Tessellation factor threshold for off-chip."},
		{"r_offchipTessellationWaveThreshold", "Domain shader wave threshold for off-chip."},
		{"r_omitUnusedRenderTargets", "Omit unused render targets to save memory.  Changing this requires a vid_restart."},
		{"r_outdoor", "Prevents snow from going indoors"},
		{"r_outdoorFeather", "Outdoor z-feathering value"},
		{"r_particleHdr", "Enable Hdr Particle Features"},
		...
		{"r_sky_fog_intensity", "Amount of sky fog fading"},
		{"r_sky_fog_max_angle", "End of angular sky fog fading"},
		{"r_sky_fog_min_angle", "Start of angular sky fog fading"},
		{"r_skyFogUseTweaks", "Enable dvar control of sky fog"},
		{"r_smaaThreshold", "SMAA edge detection threshold"},
		...
		{"r_smp_worker_thread3", "undefined"},
		{"r_smp_worker_thread4", "undefined"},
		{"r_smp_worker_thread5", "undefined"},
		{"r_smp_worker_thread6", "undefined"},
		{"r_smp_worker_thread7", "undefined"},
		...
		{"r_sssPreset", "Changes subsurface scattering quality"},
		...
		{"r_tessellationHeightScale", "Displacement height scale factor."},
		{"r_tessellationHybrid", "Hybrid per pixel displacement scale."},
		{"r_tessellationLodBias", "Displacement map lod bias."},
		...
		{"r_texFilterProbeBilinear", "Force reflection probe to use bilinear filter"},
		{"r_texShowMipMode", "Forces textures with the specified mip filtering to draw black."},
		{"r_thermalColorOffset", "Offset of the thermal colors (offset + scale*color)"},
		{"r_thermalColorScale", "Scale of the thermal colors (offset + scale*color)"},
		{"r_thermalDetailScale", "Scale of the detail that is added to the thermal map from the normal map (multiplies the detail amount from AssetManager)"},
		{"r_thermalFadeColor", "Color the thermal fades to at distance."},
		{"r_thermalFadeControl", "Select thermal fade mode"},
		{"r_thermalFadeMax", "Distance at which thermal stops fading"},
		{"r_thermalFadeMin", "Distance at which thermal starts fading"},
		{"r_tonemap", "HDR Tonemapping mode"},
		{"r_tonemapAdaptSpeed", "HDR Tonemap exposure adaptation speed"},
		{"r_tonemapAuto", "HDR Tonemapping performs auto-exposure"},
		{"r_tonemapAutoExposureAdjust", "HDR Tonemap Auto Exposure Adjust value (set to 0.0 for automatic adjustment)"},
		{"r_tonemapBlack", "HDR Filmic Tonemap black point"},
		{"r_tonemapBlend", "HDR Tonemapping blends between exposures"},
		{"r_tonemapCrossover", "HDR Filmic Tonemap crossover point"},
		{"r_tonemapDarkEv", "HDR Tonemap Dark EV"},
		{"r_tonemapDarkExposureAdjust", "HDR Tonemap Dark Exposure Adjust"},
		{"r_tonemapExposure", "HDR Tonemap exposure (in EV) override (only works in non-auto mode)"},
		{"r_tonemapExposureAdjust", "HDR Tonemap exposure adjustment (in EV, 0 is no adjustment, works like a camera where +1 reduces EV by 1)"},
		{"r_tonemapGamma", "HDR Tonemap gamma curve power"},
		{"r_tonemapHighlightRange", "HDR Tonemap dynamic range, which determines white point luminance"},
		{"r_tonemapLightEv", "HDR Tonemap Light EV"},
		{"r_tonemapLightExposureAdjust", "HDR Tonemap Light Exposure Adjust"},
		{"r_tonemapLockAutoExposureAdjust", "HDR Tonemapping lock auto exposure adjust"},
		{"r_tonemapMaxExposure", "HDR Tonemap maximum exposure (in EV)"},
		{"r_tonemapMaxExposureAdjust", "HDR Tonemap Max Exposure Adjust"},
		{"r_tonemapMidEv", "HDR Tonemap Mid EV"},
		{"r_tonemapMidExposureAdjust", "HDR Tonemap Mid Exposure Adjust"},
		{"r_tonemapMinExposureAdjust", "HDR Tonemap Min Exposure Adjust"},
		{"r_tonemapShoulder", "HDR Filmic Tonemap shoulder control (0 is linear)"},
		{"r_tonemapToe", "HDR Filmic Tonemap toe control  (0 is linear)"},
		{"r_tonemapUseCS", "HDR Tonemapping uses compute shader."},
		{"r_tonemapUseTweaks", "Override tone map LightSet settings with tweak dvar values."},
		{"r_tonemapWhite", "HDR Filmic Tonemap white point"},
		...
		{"r_umbra", "Enables Umbra-based portal culling."},
		...
		{"r_useComputeSkinning", "Enables compute shader (GPU) skinning."},
		{"r_useLayeredMaterials", "Set to true to use layered materials on shader model 3 hardware"},
		{"r_useLightGridDefaultFXLightingLookup", "Enable/disable default fx lighting lookup\n"},
		{"r_useLightGridDefaultModelLightingLookup", "Enable/disable default model lighting lookup\n"},
		{"r_useShadowGeomOpt", "Enable iwRad shadow geometry optimization. It only works when we have the data generated in iwRad."},
		{"r_useSunShadowPortals", "Enable sun shadow portals when dir light change and not using cached shadow."},
		{"r_useXAnimIK", "Enables IK animation."},
		...
		{"r_veil", "Apply veiling luminance (HDR glow)"},
		{"r_veilAntialiasing", "Veil antialiasing mode (downsample technique used for first mip)."},
		{"r_veilBackgroundStrength", "Strength of background when applying veiling luminance (HDR glow)"},
		{"r_veilFalloffScale1", "Controls the size of individual Gaussians (Gaussians 4-6 in XYZ, where Gaussian 6 is the wider one)"},
		{"r_veilFalloffScale2", "Controls the size of individual Gaussians (Gaussians 4-6 in XYZ, where Gaussian 6 is the wider one)"},
		{"r_veilFalloffWeight1", "Controls the weight of individual Gaussians (Gaussians 4-6 in XYZ, where Gaussian 6 is the wider one)"},
		{"r_veilFalloffWeight2", "Controls the weight of individual Gaussians (Gaussians 4-6 in XYZ, where Gaussian 6 is the wider one)"},
		{"r_veilFilter", "Changes the veil filtering mode"},
		{"r_veilPreset", "Changes veil sampling quality"},
		{"r_veilRadius", "Controls the radius of the first Gaussian in virtual pixels (remaining Gaussians follow proportionally)."},
		{"r_veilStrength", "Strength of veiling luminance (HDR glow)"},
		{"r_veilUseTweaks", "Override veil LightSet settings with tweak dvar values."},
		{"r_velocityPrepass", "Perform velocity rendering during the depth prepass"},
		{"r_viewModelLightAmbient", ""},
		{"r_viewModelPrimaryLightTweakDiffuseStrength", "Tweak the diffuse intensity for view model primary lights"},
		{"r_viewModelPrimaryLightTweakSpecularStrength", "Tweak the specular intensity for view model primary lights"},
		{"r_viewModelPrimaryLightUseTweaks", ""},
		{"r_volumeLightScatter", "Enables volumetric light scattering"},
		{"r_volumeLightScatterAngularAtten", "Distance of sun from center of screen before angular attenuation starts for god rays"},
		{"r_volumeLightScatterBackgroundDistance", "Distance at which pixels are considered background for volume light scatter effect"},
		{"r_volumeLightScatterColor", ""},
		{"r_volumeLightScatterDepthAttenFar", "Pixels >= than this depth recieve full volume light scatter."},
		{"r_volumeLightScatterDepthAttenNear", "Pixels <= than this depth recieve no volume light scatter."},
		{"r_volumeLightScatterEv", "Light intensity (in EV) for volumetric light scattering"},
		{"r_volumeLightScatterLinearAtten", "Coefficient of linear attenuation of god rays"},
		{"r_volumeLightScatterQuadraticAtten", "Coefficient of quadratic attenuation of god rays)"},
		{"r_volumeLightScatterUseTweaks", "Enables volumetric light scattering tweaks"},
		...
		{"r_wideTessFactorsThreshold", "If a surface has more than this many triangles, process triangles in parallel instead of surfaces."},
		...
		{"ragdoll_link_to_moving_platform", "Enable client-side linking of ragdolls to script brush models when they go idle."},
		{"ragdoll_max_life", "Max lifetime of a ragdoll system in msec"},
		{"ragdoll_max_simulating", "Max number of simultaneous active ragdolls - archived"},
		{"ragdoll_max_stretch_pct", "Force ragdoll limbs to not stretch more than this percentage in one frame"},
		{"ragdoll_mp_limit", "Max number of simultaneous active ragdolls - archived"},
		{"ragdoll_mp_resume_share_after_killcam", "Msec after returning from killcam that splitscreen players will share ragdolls again."},
		{"ragdoll_resolve_penetration_bias", "Bias value on force to push ragdolls out of environment."},
		...
		{"s_aggregate_ping_offset", "offset to apply to aggregate ping values"},
		{"s_aggregate_ping_scale", "8-bit fixed-point aggregate ping scaler value"},
		{"s_avg_max_weighting", "weighting from 0-256 of party average ping vs. worst ping"},
		{"s_ds_pingclient_reping_wait_db", "wait this# of frames for the db thread to settle down before repinging"},
		{"s_use_aggregate_datacenter_pings", "use newer system for aggregating party pings"},
		...
		{"scr_game_lockspectatorpov", "Lock spectator mode globally, 0=freelook/unlocked, 1=first_person, 2=third_person"},
		...
		{"showDebugAmmoCounter", "Show the debug ammo counter when unable to show ar ammo counter"},
		{"showPlaylistTotalPlayers", "Toggle the display of the total number of players in a playlist and online"},
		{"sm_cacheSpotShadows", "Cache spot shadow maps, improves shadow map performance at the cost of memory (requires vid_restart)"},
		{"sm_cacheSpotShadowsEnabled", "Enables caching of spot shadows."},
		{"sm_cacheSunShadow", "Cache sun shadow map, improves shadow map performance at the cost of memory (requires vid_restart)"},
		{"sm_cacheSunShadowEnabled", "Enables caching of sun-based shadows."},
		{"sm_cameraOffset", ""},
		{"sm_dynlightAllSModels", "Enable, from script, rendering all static models in dynamic light volume when shadow mapping"},
		...
		{"sm_lightScore_spotProjectFrac", "When picking shadows for primary lights, measure distance from a point this far in front of the camera."},
		{"sm_maxLightsWithShadows", "Limits how many primary lights can have shadow maps"},
		...
		{"sm_polygonOffsetClamp", "Shadow map offset clamp"},
		{"sm_polygonOffsetPreset", "Shadow map polygon offset preset."},
		{"sm_polygonOffsetScale", "Shadow map offset scale"},
		{"sm_qualitySpotShadow", "Fast spot shadow"},
		{"sm_shadowUseTweaks", "Override shadow LightSet settings with tweak dvar values."},
		{"sm_spotDistCull", "Distance cull spot shadows"},
		{"sm_spotEnable", "Enable spot shadow mapping from script"},
		{"sm_spotFilterRadius", "Spot soft shadows filter radius"},
		...
		{"sm_sunFilterRadius", "Sun soft shadows filter radius"},
		{"sm_sunSampleSizeNear", "Shadow sample size"},
		{"sm_sunShadowBoundsMax", "Max Shadow Bounds"},
		{"sm_sunShadowBoundsMin", "Min Shadow Bounds"},
		{"sm_sunShadowBoundsOverride", "Override Shadow Bounds"},
		{"sm_sunShadowCenter", "Sun shadow center, 0 0 0 means don't override"},
		{"sm_sunShadowCenterMode", "When false center value only used for far map, when true sets both maps"},
		{"sm_sunShadowScale", "Sun shadow scale optimization"},
		{"sm_sunShadowScaleLocked", "Lock usage of sm_sunShadowScale at 1"},
		{"sm_usedSunCascadeCount", "How many shadow cascade we are using"},
		{"snd_allowHeadphoneHRTF", "Enable HRTF over headphones"},
		{"snd_announcerDisabled", "Disable all in-game announcers"},
		{"snd_announcerVoicePrefix", "Local mp announcer voice to use"},
		{"snd_battlechatterDisabled", "Disable all in-game battle chatter"},
		{"snd_cinematicVolumeScale", "Scales the volume of Bink videos."},
		{"snd_detectedSpeakerConfig", "speaker configuration:\n0: autodetect\n1: mono\n2: stereo\n4: quadrophonic\n6: 5.1 surround\n8: 7.1 surround"},
		{"snd_dopplerAuditionEnable", "Enables doppler calculation preview mode"},
		{"snd_dopplerBaseSpeedOfSound", "The base speed of sound used in doppler calculation"},
		...
		{"snd_peakLimiterDecay", "Peak limiter compression decay ratio."},
		{"snd_peakLimiterSustainFrames", "Number of frames to sustain the limiter peak. 1 frame = 10 msec."},
		{"snd_premixVolume", "Game sound pre-mix volume"},
		{"snd_reverbZoneOutsideFactor", "When a 3d sound is played in a different reverb zone than the player, this factor will be applied to its wet level."},
		...
		{"snd_speakerConfig", "speaker configuration:\n0: autodetect\n1: mono\n2: stereo\n4: quadrophonic\n6: 5.1 surround\n8: 7.1 surround"},
		{"snd_touchStreamFilesOnLoad", "Check whether stream sound files exist while loading"},
		{"snd_useOldPanning", "Use old and busted panning"},
		{"snd_virtualChannelInfo", "Display virtual voice info."},
		...
		{"svwp", "playerdata server write protection: 0 = disable, 1 = silent, 2 = kick"},
		{"syncTimeTimeout", "default timeout for sync time task (in seconds)"},
		...
		{"sys_quitMigrateTime", "Time in msec to wait for host migration when user closes the window"},
		...
		{"systemlink_host", "Local client is hosting system link game"},
		{"tb_report", "tb event record"},
		{"team_rebalance", "rebalance"},
		{"teambalance_option", "Selects active teambalance algorithm. 0 = heuristic 1 = exhaustive"},
		{"theater_active", "Are we allowed to show theater or not."},
		{"thermal_playerModel", "Model to draw for players when in thermal vision mode"},
		{"thermalBlurFactorNoScope", "Amount of blur to use when drawing blur through a weapon's thermal scope."},
		{"thermalBlurFactorScope", "Amount of blur to use when drawing blur through a weapon's thermal scope."},
		{"tokensEnabled", "Is token economy enabled"},
		...
		{"tracer_stoppingPowerColor1", "The 1st color of a bullet tracer when using explosive bullets"},
		{"tracer_stoppingPowerColor2", "The 2nd color of a bullet tracer when using explosive bullets"},
		{"tracer_stoppingPowerColor3", "The 3rd color of a bullet tracer when using explosive bullets"},
		{"tracer_stoppingPowerColor4", "The 4th color of a bullet tracer when using explosive bullets"},
		{"tracer_stoppingPowerColor5", "The 5th color of a bullet tracer when using explosive bullets"},
		{"tracer_stoppingPowerOverride", "When turned on, will apply an override to the tracer settings when shooting explosive bullets."},
		{"tracer_stoppingPowerWidth", "The width of a bullet tracer when using explosive bullets"},
		{"tracer_thermalWidthMult", "The multiplier applied to the base width when viewed in thermal vision"},
		{"transients_verbose", "Verbose logging information for transient fastfiles."},
		{"triggerDLCEnumerationOnSocialConfigLoad", "Triggers a new DLC enumeration after social config has loaded."},
		{"ui_allow_controlschange", ""},
		{"ui_allow_teamchange", ""},
		{"ui_autodetectGamepad", "undefined"},
		{"ui_autodetectGamepadDone", "undefined"},
		{"ui_bigFont", "Big font scale"},
		{"ui_blurAmount", "Max amount to blur background menu items."},
		{"ui_blurDarkenAmount", "Amount to darken blurred UI."},
		{"ui_blurTime", "Time in milliseconds to fade in/out the blur."},
		...
		{"ui_contextualMenuLocation", "Contextual menu location from where you entered the store."},
		...
		{"ui_disableInGameStore", "This will disable the ingame store button on the xbox live menu."},
		{"ui_disableTokenRedemption", "This will disable the token redemption option in the in-game store menu."},
		{"ui_drawCrosshair", "Whether to draw crosshairs."},
		{"ui_editSquadMemberIndex", "Which squad member is currently being edited"},
		...
		{"ui_hud_obituaries", ""},
		{"ui_inactiveBaseColor", "The local player's rank/stats font color when shown in lobbies and parties"},
		{"ui_inactivePartyColor", ""},
		{"ui_inGameStoreOpen", "is the InGameStore open"},
		{"ui_inhostmigration", ""},
		{"ui_joinGametype", "Game join type"},
		{"ui_loadMenuName", "Frontend menu will start on this level instead of lockout"},
		{"ui_mapname", "Current map name"},
		{"ui_mapvote_entrya_gametype", "Primary map vote entry game type"},
		{"ui_mapvote_entrya_mapname", "Primary map vote entry map name"},
		{"ui_mapvote_entryb_gametype", "Secondary map vote entry game type"},
		{"ui_mapvote_entryb_mapname", "Secondary map vote entry map name"},
		{"ui_maxclients", "undefined"},
		{"ui_missingMapName", "Name of map to show in missing content error"},
		{"ui_mousePitch", ""},
		{"ui_multiplayer", "True if the game is multiplayer"},
		{"ui_myPartyColor", "Player name font color when in the same party as the local player"},
		...
		{"vehicle_pathsmooth", "Smoothed vehicle pathing."},
		...
		{"winvoice_mic_outTime", "Microphone voice amount of silence before we cut the mic"},
		{"winvoice_mic_reclevel", "Microphone recording level"},
		{"winvoice_mic_scaler", "Microphone scaler value"},
		{"winvoice_mic_threshold", "Microphone voice threshold"},
		{"winvoice_save_voice", "Write voice data to a file"},
		{"xanim_disableIK", "Disable inverse kinematics solvers"},
		{"xblive_competitionmatch", "MLG Rules?"},
